...
def create_geom_foil(geom):
    g = geom
    g.X, g.Y = nc.get_naca45("0012", g.npan, nc.spc_cos)
    create_geom(g)
...
def plot_streamlines(dev, frm, geom, xmin, xmax, ymin, ymax, nx, ny):
    ...
    vgl.print_top_center(dev, f"\alpha: {np.rad2deg(g.alpha):2.2f}")
...
def run_spm(t):
    ...
    spm_circ = SPMGeom(1,aoa_d, radius, 0, npan, radius)
    spm_foil = SPMGeom(1,aoa_d, radius, 0, npan, radius)
    create_geom_foil(spm_foil)
...
def save():
    ...
    dev = vgl.DeviceIMG("", fmm.get_gbbox(), 100)
